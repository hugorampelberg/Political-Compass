...
Les champs axis_scores et axis_similarity n’ont pas le même sens : ne les confonds jamais. Le score global repose à 100 % sur la proximité question par question. axis_coordinate_similarity reste un indicateur diagnostique et ne contribue pas au classement.
...
function validatePayload(payload) {
  ...
  if (
    ...
    payload.question_responses.length !== 88
  ) {
    return false;
  }
  for (const item of payload.question_responses) {
    if (!item || typeof item !== "object") return false;
    if (!Number.isInteger(item.id) || item.id < 1 || item.id > 88) return false;
