...
const AI_ENABLED = Boolean(API_KEY) && SERVICE_TIER === "paid";
...
function textLength(value, max, min = 0) {
  return typeof value === "string" && value.length >= min && value.length <= max;
}

function numericRecord(value, expectedSize, minimum, maximum) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return false;
  const entries = Object.entries(value);
  return entries.length === expectedSize && entries.every(([, number]) => (
    Number.isFinite(number) && number >= minimum && number <= maximum
  ));
}

function validRanking(list, minimumLength, maximumLength) {
  if (!Array.isArray(list) || list.length < minimumLength || list.length > maximumLength) return false;
  return list.every((item, index) => (
    item
    && typeof item === "object"
    && item.rank === index + 1
    && textLength(item.name, 200, 1)
    && Number.isFinite(item.score)
    && item.score >= 0
    && item.score <= 100
    && numericRecord(item.axis_scores, 6, 0, 100)
  ));
}
...
function validatePayload(payload) {
  ...
  if (payload.consent_version !== "gemini-sensitive-data-v3") return false;
  if (!numericRecord(payload.axis_scores, 6, -10, 10)) return false;
  ...
  if (payload.question_count !== responseCount) return false;
  if (payload.quiz_mode !== (responseCount === 40 ? "quick" : "full")) return false;

  const expectedIds = responseCount === 40
    ? QUICK_QUESTION_IDS
    : new Set(Array.from({ length: 80 }, (_, index) => index + 1));
  const receivedIds = new Set();
  for (const item of payload.question_responses) {
    if (!item || typeof item !== "object") return false;
    if (!Number.isInteger(item.id) || !expectedIds.has(item.id) || receivedIds.has(item.id)) return false;
    receivedIds.add(item.id);
    if (!Number.isInteger(item.answer) || item.answer < -3 || item.answer > 3) return false;
    if (!textLength(item.question, 1_200, 1) || !textLength(item.theme || "", 200, 1)) return false;
    if (!textLength(item.answer_label || "", 120, 1) || !textLength(item.comment || "", 6_000)) return false;
  }
  ...
  if (payload.open_answers.some(x => !x || !textLength(x.question || "", 1_200, 1) || !textLength(x.answer || "", 8_000))) return false;
  if (!validRanking(payload.party_ranking, 1, 20)) return false;
  if (!validRanking(payload.government_ranking, 1, 12)) return false;
...
async function analyse(payload) {
  ...
  if (SERVICE_TIER !== "paid") throw geminiError("GEMINI_PAID_TIER_REQUIRED", {
    code: "GEMINI_PAID_TIER_REQUIRED", status: 503
  });
...
export {
  API_KEY, MODEL, FALLBACK_MODEL, SERVICE_TIER, AI_ENABLED, MAX_BODY_BYTES,
