...
const STORAGE_KEY = 'boussole-politique-state-v2';
    const QUIZ_MODES = { FULL:'full', QUICK:'quick' };
    const QUICK_QUESTION_IDS = new Set([1,3,5,7,9,10,11,14,19,20,21,24,25,26,27,28,29,32,34,35,38,39,40,42,46,49,50,56,63,64,66,67,70,71,72,74,76,77,78,79]);
    ...
    function saveState(){ state.updatedAt=Date.now(); localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); refreshResume(); }
    function clearState(){ localStorage.removeItem(STORAGE_KEY); state=initialState(); results=null; aiAnalysisResult=null; refreshResume(); }
    ...
    function refreshResume(){
      const count=answeredActiveCount(state), total=activeQuestionCount(state);
      const visible=count>0 && !state.completed;
      $('#resume-card').classList.toggle('hidden',!visible);
      $('#top-progress-btn').classList.toggle('hidden',!visible);
      if(visible) $('#resume-copy').textContent=`${modeLabel(state)} · ${count} réponse${count>1?'s':''} enregistrée${count>1?'s':''} sur ${total}`;
    }
    ...
    function startFresh(mode=QUIZ_MODES.FULL){ state=initialState(mode); results=null; aiAnalysisResult=null; saveState(); renderPriority(); showScreen('priority'); }
    function resume(){ if(state.priority===null){ renderPriority(); showScreen('priority'); return; } const firstMissing=firstMissingActivePosition(); state.current=firstMissing>=0?firstMissing:Math.min(state.current,activeQuestionCount()-1); showScreen('quiz'); renderQuestion(); }
    ...
    function renderQuestion(){
      ...
      if(explanationText){
        explanation.innerHTML=q.source
          ? `${esc(explanationText)} <a href="${esc(q.source)}" target="_blank" rel="noopener">Source officielle</a>`
          : esc(explanationText);
    ...
    function axisMassesForMode(){
      return calculateAxisMasses(DATA.axes, DATA.questions, activeQuestionIndexes());
    }
    function axisWeights(){ return calculateAxisWeights(DATA.axes, axisMassesForMode(), state.priority); }
    function axisScoresFromAnswers(answers){
      return calculateAxisScores(DATA.axes, DATA.questions, activeQuestionIndexes(), answers);
    }
    ...
    function detailedAxisSimilarity(entity, axisKey) {
      return calculateDetailedAxisSimilarity({
        questions: DATA.questions,
        questionIndexes: activeQuestionIndexes(),
        axisKey,
        userAnswers: state.answers,
        entityResponses: entity.responses,
    ...
    function makeSummary(scores, topParty, topGov){
      ...
      return `Votre profil est surtout ${d[0].text}, avec une sensibilité également ${d[1].text}. Votre meilleure correspondance parmi les partis est ${topParty.shortName} (${fmt(topParty.global)} %), tandis que le gouvernement le plus proche est ${topGov.shortName} (${fmt(topGov.global)} %).`;
    }
    ...
    function driversFor(entity){
      const items=activeQuestions().map(({q,i})=>{
        ...
        const distance = politicalDistance(user, party);
        const imp = questionImportance(q) * (entity.confidence[i] || 0);
    ...
    async function checkAIAvailability(){
      ...
      if(location.protocol==='file:'){
        ...
        help.textContent='Ouvrez le projet avec le serveur local (npm run dev) ou utilisez la version déployée : la clé Gemini doit rester hors du navigateur.';
      ...
      try{
        ...
        help.textContent='Le questionnaire fonctionne normalement. L’analyse exige une clé GEMINI_API_KEY et la confirmation GEMINI_SERVICE_TIER=paid sur un compte Gemini réellement payant.';
    ...
    $('#edit-answers-btn').addEventListener('click',()=>{ state.completed=false; state.current=0; aiAnalysisResult=null; saveState(); showScreen('quiz'); renderQuestion(); });
    ...
    document.addEventListener('keydown',e=>{
      ...
      const isInteractive=target instanceof Element && Boolean(
        ...
        || target.closest('input, textarea, select, button, summary, a[href], [contenteditable], [role="button"]')
      );
      if(e.defaultPrevented || e.repeat || e.isComposing || e.ctrlKey || e.metaKey || e.altKey || isInteractive) return;
      if(/^[1-7]$/.test(e.key)){
        e.preventDefault();
        answerQuestion(Number(e.key)-4);
        return;
      }
      if(e.key==='ArrowLeft'){
        e.preventDefault();
        prevQuestion();
      }else if(e.key==='ArrowRight' && state.answers[activeOriginalIndex()]!==null){
        e.preventDefault();
        nextQuestion();
      }
    });
